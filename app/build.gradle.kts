...
android {
    namespace = "ir.hfathi.smart_gallery"
    compileSdk = 34

    defaultConfig {
        applicationId = "ir.hfathi.smart_gallery"
        minSdk = 30
        targetSdk = 34
        versionCode = 20111
        versionName = "2.0.1"
...
//        archivesName.set("Gallery-${versionName}_$gitHeadVersion")
    ...
    buildTypes {
        getByName("debug") {
            ...
            manifestPlaceholders["appProvider"] = "ir.hfathi.gallery.debug.media_provider"
            buildConfigField(
                "String",
                "CONTENT_AUTHORITY",
                "\"ir.hfathi.gallery.debug.media_provider\""
            )
        }
        getByName("release") {
            manifestPlaceholders += mapOf(
                "appProvider" to "ir.hfathi.gallery.media_provider"
            )
            ...
            buildConfigField("String", "MAPS_TOKEN", getApiKey())
            buildConfigField("String", "CONTENT_AUTHORITY", "\"ir.hfathi.gallery.media_provider\"")
        }
        create("staging") {
            ...
            manifestPlaceholders["appProvider"] = "ir.hfathi.staging.debug.media_provider"
            buildConfigField(
                "String",
                "CONTENT_AUTHORITY",
                "\"ir.hfathi.staging.debug.media_provider\""
            )
        }
    }
